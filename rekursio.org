...
** Taulukointi
   ...
   tallennetaan taulukkoon kun se lasketaan ensimmäisen
   kerran. Alla olevassa ohjelmassa tämä taulukko on ~f~. Taulukko
   alustetaan ohjelman alussa luvuilla \(-1\), jotta ohjelmassa
   voidaan tunnistaa onko arvo jo laskettu.

   Alla olevassa ohjelmassa lasketaan ja tulostetaan arvo \(f_{20}.\)
   Tällöin taulukkoon ~f~ täytyy varata tilaa 21 luvulle, koska
   ~f[20]~ on taulukon ~f~ 21. luku.
   ...
   #+BEGIN_SRC processing :exports both
     ...
     void draw ()
     {
       ...
       text (fib (n), width / 2.0, height / 2.0);
     }
