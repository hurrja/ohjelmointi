...
** Esimerkki
   ...
   #+BEGIN_SRC processing :noweb yes
     ...
     final float[] X_T = {<<taulukko-ympyrat-annetut-x>>};
...
** Taulukkovakio
   ...
   nollasta/. Jos taulukon nimi on ~X_T~, niin taulukon
   ensimmäinen luku on
   #+BEGIN_EXAMPLE
   X_T [0]
   #+END_EXAMPLE
   ...
   nimi on ~X_T~, niin taulukon pituus saadaan ohjelmassa
   lausekkeella 
   #+BEGIN_EXAMPLE
   X_T.length
   #+END_EXAMPLE
   ...
   call_taulukko-ympyrat-annetut-x() ovat taulukossa ~X_T~, niin
   ...
   #+BEGIN_SRC asymptote :file taulukko-xkoordinaatit-taulukossa.svg :noweb yes
     ...
     for (int i = 0; i < coordinates.length; ++i)
       {
         tableSpecStr += "c";
         headlineStr += "&" + "\texttt{X\_T} [" + (string) i + "]";
...
** Taulukkomuuttuja
   ...
   #+BEGIN_QUOTE
   Desimaalukuja sisältävä taulukkomuuttuja ~x_t~ määritellään
   lausekkeella
   #+BEGIN_EXAMPLE
   float[] x_t;
   #+END_EXAMPLE
