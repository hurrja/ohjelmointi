...
** Esimerkki
   ...
   mahdollisimman ympyrää piirtoikkunaan, jonka leveys on suurempi
   kuin korkeus. Kahden mustan neliön välissä on yhtä suuri sininen
   neliö. Neliöiden sivun pituus on yhtä suuri kuin piirtoikkunan
   korkeus.
   #+NAME: vakio-mustat-siniset-neliot
   #+BEGIN_SRC processing
     size (990, 100);
     colorMode (HSB, 100);
     background (0); // musta tausta
     ...
     while (n <= ympyroita)
     {
       if (n % 2 == 1) // 1. ja muut parittomat sinisellä
         fill (62, 100, 100);
       else
         fill (17, 100, 100);
   ...
   - Kokonaislukumuuttuja ~n~ ilmaisee /jo piirrettyjen/ neliöiden
     lukumäärän, ja sen arvo on alussa 0.
   - ~while~-silmukkaa toistetaan niin kauan, kun piirrettyjen
     neliöiden lukumäärä on pienempi kuin piirrettävien lukumäärä.
   - Piirtoikkunan tausta asetetaan siniseksi, jolloin vain mustat
     neliöt tarvitsee piirtää.
   - Neliö on musta silloin, kun jo piirrettyjen neliöiden määrä on
     parillinen.
   - Jo piirrettyjen neliöiden leveys on ~n*sivu~, joten tämä on myös
     seuraavan neliön vasemman yläkulman \(x\)-koordinaatti.

   #+BEGIN_SRC processing :noweb yes :exports code
   <<vakio-mustat-siniset-neliot>>
   #+END_SRC
