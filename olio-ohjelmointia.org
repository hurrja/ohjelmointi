...
** Esimerkki
   ...
   #+END_SRC
** Esimerkki
   Luokka voidaan myös määritellä rekursiivisesti. /Binääripuu/ on
   ...
   konstruktorin parametri ~suurin~.
   - 
   #+BEGIN_SRC processing
     class Binaaripuu
     {
       ...
       {
         assert (tasoja > 0); // varmistetaan, että parametrin arvo järkevä
         arvo = floor (random (suurin + 1)); // solmun satunnainen arvo
    
         // rekursion lopetusehto
         if (tasoja == 1)
         {
           vasen = null;
           oikea = null;
                    ...
                    float askel)
       {
         ...
         if (vasen != null)
         {
           assert (oikea != null); // jos on vasen, pitäisi olla oikeakin
